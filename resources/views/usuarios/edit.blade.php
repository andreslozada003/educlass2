...
<div class="flex items-center justify-between">
    <div>
        ...
        <p class="text-gray-500">{{ $usuario->name }}</p>
    </div>
    <a href="{{ route('usuarios.show', $usuario) }}" class="text-gray-600 hover:text-gray-700">
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
    <form action="{{ route('usuarios.update', $usuario) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="role" id="role" required
                    ...
                    <option value="{{ $rol->name }}" {{ old('role', $usuario->getRoleNames()->first()) == $rol->name ? 'selected' : '' }}>{{ ucfirst($rol->name) }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                @if($usuario->avatar)
                <div class="mb-2">
                    <img src="{{ asset('storage/' . $usuario->avatar) }}" alt="" class="h-20 rounded-lg">
                </div>
                @endif
        ...
        <div class="flex justify-end space-x-4 mt-6 pt-6 border-t border-gray-200">
            <a href="{{ route('usuarios.show', $usuario) }}" class="px-6 py-3 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition-colors">
                Cancelar
            </a>
